...
def scheduled_sync_gmail(self) -> dict:
    from apps.integrations.models import Integration

    integrations = Integration.objects.filter(source="gmail", is_active=True)
    results = []
    for integration in integrations:
        task = full_sync_gmail.delay(str(integration.id), triggered_by="scheduled")
        results.append({"integration_id": str(integration.id), "task_id": task.id})
    return {"dispatched": len(results)}


@shared_task(queue="sync", bind=True, max_retries=2)
def full_sync_gmail(self, integration_id: str, triggered_by: str = "scheduled") -> dict:
    ...
    try:
        connector = GmailConnector(integration_id, str(integration.organization_id))
